        ...
        <div className="glass-card p-6 md:p-8 rounded-[1.5rem] relative z-10 bg-black/40 backdrop-blur-3xl shadow-neon overflow-hidden">
          ...
          <div className="text-center mb-6 relative">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[8px] font-bold uppercase tracking-[0.4em] text-neon-pink mb-3">
              <ShieldCheck className="w-3 h-3 text-neon-pink" /> Security Level: Omega
            </div>
            <h2 className="text-2xl md:text-3xl font-display font-bold text-white mb-2 uppercase tracking-tighter">Initialize Link</h2>
            <p className="text-xs text-muted-foreground/80 font-medium italic">Synchronize your neural nodes with the platform.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-4">
              <label className="text-[10px] font-bold uppercase tracking-[0.5em] text-muted-foreground/40 block ml-2">Neural Address</label>
              <div className="relative group">
                ...
                <input
                  ...
                  required
                  className="w-full bg-white/5 border border-white/10 rounded-2xl pl-16 pr-6 py-5 text-white placeholder:text-muted-foreground/30 focus:outline-none focus:border-neon-pink/50 focus:bg-white/10 transition-all font-mono text-sm tracking-wide"
                  placeholder="IDENTIFIER@ENGINE.NET"
                />
                <div className="absolute inset-0 rounded-2xl bg-neon-pink/5 opacity-0 group-focus-within:opacity-100 pointer-events-none transition-opacity" />
              </div>
            </div>

            <div className="space-y-4">
              <div className="flex justify-between items-center ml-2">
                <label className="text-[10px] font-bold uppercase tracking-[0.5em] text-muted-foreground/40 block">Encryption Key</label>
                <Link href="#" className="text-[10px] font-bold uppercase tracking-widest text-neon-pink hover:text-neon-rose transition-colors underline underline-offset-4 decoration-neon-pink/30">Lost Link?</Link>
              </div>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none">
            ...
            <AnimatePresence>
              {error && (
                <motion.div
                  ...
                  className="bg-neon-crimson/10 border border-neon-crimson/20 text-neon-crimson text-[10px] font-bold uppercase tracking-widest rounded-xl p-4 flex items-center gap-3 overflow-hidden"
                >
            ...
            >
              ...
              <div className="relative py-3.5 rounded-[1rem] bg-neon-crimson text-white text-[10px] font-bold uppercase tracking-[0.2em] hover:scale-[0.98] active:scale-95 disabled:opacity-50 disabled:scale-100 transition-all flex items-center justify-center gap-3 shadow-neon overflow-hidden">
                {isLoading ? (
                  ...
                  <>
                    Authorize Connection <Fingerprint className="w-4 h-4 group-hover:scale-125 transition-transform duration-500" />
                  </>
                )}
              </div>
            </button>
          ...
          <div className="mt-6 pt-5 border-t border-white/5 flex flex-col items-center gap-3">
            <p className="text-[10px] text-muted-foreground font-medium italic">
              New explorer? <Link href="/register" className="text-white font-bold uppercase tracking-widest hover:text-neon-pink hover:underline transition-all underline-offset-4 ml-2">Create Account</Link>
            </p>
          </div>
        </div>
      </motion.div>
